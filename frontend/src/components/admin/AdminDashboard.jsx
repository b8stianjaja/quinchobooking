...
function AdminDashboard({ currentAdminUser, onLogout }) {
  ...
  const fetchBookings = useCallback(async () => {
    setIsLoading(true); // Asegúrate de restablecer isLoading
    try {
      ...
      if (err.message?.toLowerCase().includes('unauthorized') || err.message?.toLowerCase().includes('no active session')) {
        if (onLogout) onLogout(navigate); // Llama a onLogout si hay error de autorización o sesión
      }
    } finally {
      setIsLoading(false);
    }
  }, [onLogout, navigate]); // Añade navigate a las dependencias si lo usas en onLogout

  useEffect(() => {
    if (currentAdminUser) {
      fetchBookings();
    } else {
       // Si no hay usuario (ej. después de logout o al cargar sin sesión), no intentes cargar
       setIsLoading(false); // Detiene la carga si no hay usuario
       setBookings([]); // Limpia las reservas
    }
    ...
    <div className="min-h-screen bg-gray-50">
       <header className="bg-white shadow-sm sticky top-0 z-40">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-20">
          ...
          <button
            onClick={() => onLogout(navigate)} // Asegúrate que onLogout recibe navigate si lo necesita
      ...
      <main className="container mx-auto p-4 sm:p-6 lg:p-8">
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          <StatCard title="Total de Reservas" value={stats.total} icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-orange-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}> <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /> </svg>} />
          <StatCard title="Pendientes de Confirmación" value={stats.pending} icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-yellow-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}> <path strokeLinecap="round" strokeLinejoin="round" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /> </svg>} />
          <StatCard title="Eventos Confirmados" value={stats.confirmed} icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}> <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /> </svg>} />
        </section>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
          <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
            ...
            <div className="relative w-full sm:w-auto">
              ...
              <input type="text" placeholder="Buscar por nombre o teléfono..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="block w-full rounded-md border-gray-300 py-2 pl-10 pr-3 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm" />
            </div>
          </div>
